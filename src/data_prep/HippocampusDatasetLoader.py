...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        # <YOUR CODE GOES HERE>
        normlized_image = np.asarray(image).astype(np.single) / 0xff
        ...
        # print(f'1: {image.shape[0]}, 2: {y_shape}, 3: {z_shape}')
        # print('Image name: ', f)
...
#         if(f == "hippocampus_010.nii.gz"): 
# #             print(f'Image: {image} and label: {label}')
#             print(f'Image: {image.shape} and label: {label.shape}')
